...
export async function exportLeaseReportHTML(data: AnalysisData): Promise<void> {
  try {
    ...
    const addressSlug = data.address ? 
      ...
      'lease';
    const filename = `LeaseChat_Report_${addressSlug}_${timestamp}.pdf`;
...
LeaseChat - Lease Analysis Report
...
Generated by LeaseChat | University of Chicago Law School AI Lab
    `;
    ...
    a.href = url;
    a.download = `LeaseChat_Report_${new Date().toISOString().split('T')[0]}.txt`;
