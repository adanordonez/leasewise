...
export async function exportLeaseReport(data: AnalysisData): Promise<void> {
  ...
  doc.setFontSize(24);
  doc.setFont('helvetica', 'bold');
  doc.text('LeaseWise', pageWidth / 2, 20, { align: 'center' });
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (i === totalPages) {
      doc.setFontSize(7);
      doc.setTextColor(...mediumGray);
      const disclaimer = 'Disclaimer: This is a beta product. It uses AI and may provide wrong information. LeaseWise provides legal information, not legal advice. Using LeaseWise does not create an attorney-client relationship. LeaseWise is not your attorney.';
    ...
    doc.text('Generated by LeaseWise | UChicago Law School AI Lab', 14, pageHeight - 10);
  }

  // Save the PDF
  const filename = `LeaseWise_Report_${data.userName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
}
